...
- **[[Database Instance]]:** Phân biệt giữa *Database* (các file vật lý trên đĩa) và *Instance* (bộ nhớ RAM + các tiến trình ngầm).
- **[[Buffer Cache]]:** Vùng nhớ lưu trữ các Block dữ liệu thường xuyên truy xuất, giải thuật thay thế trang LRU (Least Recently Used).
- **Redo Log / WAL (Write-Ahead Logging):** Cơ chế đảm bảo tính bền vững (Durability) của giao dịch bằng cách ghi tuần tự (Sequential I/O).
