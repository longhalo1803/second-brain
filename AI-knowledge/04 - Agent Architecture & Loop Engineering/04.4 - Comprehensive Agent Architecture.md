...
- **Cơ chế truy xuất kết hợp (Hybrid Retrieval)**:
  - **SQL Query**: Khi cần lọc theo mốc thời gian hoặc thuộc tính cụ thể (ví dụ: _10 tin nhắn gần nhất với khách hàng X_).
  - **RAG / Semantic Search**: Khi cần tìm kiếm sự kiện theo ngữ nghĩa (ví dụ: _20 trường hợp khiếu nại chất lượng sản phẩm chưa được giải quyết_).
